...
public class ThemSachVaoGioHangUITest {
    ...
    public void testShowSachDetails() {
        ...
        assertEquals("NXB001", themSachVaoGioHangUI.showNXB.getText()); // Kiểm tra mã NXB hiển thị đúng
        assertEquals("TG001", themSachVaoGioHangUI.showTenTG.getText()); // Kiểm tra mã tác giả hiển thị đúng
        assertEquals("10000.0", themSachVaoGioHangUI.showDonGia.getText()); // Kiểm tra đơn giá hiển thị đúng
        assertEquals("2", themSachVaoGioHangUI.showSoLuong.getText()); // Kiểm tra số lượng hiển thị đúng
        assertEquals("20000.0", themSachVaoGioHangUI.showThanhTien.getText()); // Kiểm tra thành tiền hiển thị đúng (2 * 10000)
    }
}
